...
async def handle_text_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not access_token or not post_url or not delay:
        await update.message.reply_text("❌ Missing token, post URL, or delay.")
        return ConversationHandler.END

    # Wait for the specified delay before posting
    await update.message.reply_text(f"⏳ Waiting for {delay} seconds before posting...")
    await asyncio.sleep(delay)

    # Send the sticker via Facebook
    ...
    if success:
        ...
        await update.message.reply_text(f"❌ Failed to send sticker.")
